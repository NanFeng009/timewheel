...
{
        printf("remove from the runqueue \n");
    if (!this->running) {
        return false;
    }
...
void task::setPaused()
{
    if (running) {
        if (!remove_from_runqueue()) {
            printf("Tried to remove a running call that wasn't running!\n");
            assert(0);
        }
    } else {   
        paused_tasks.remove_paused_task(this);
...
task_list *timewheel::task2list(task *task)
{
    ...
    if (wheel_base > clock_tick) {
        printf("wheel_base is %lu, clock_tick is %lu - expected wheel_base to be less than or equal to clock_tick", wheel_base, clock_tick);
    ...
    if (fits_in_first_wheel) {
        ...
        printf("Attempted to schedule a task too far in the future");
        return NULL;
    }
}
